--TEST--
Compression H1 streaming: a Brotli chunk reaches the client before the stream ends (#170)
--EXTENSIONS--
true_async_server
true_async
--SKIPIF--
...
$head = str_repeat("progressive brotli chunk\n", 400);
$tail = str_repeat("tail written after the gate\n", 20);
$gate = false;

/* PHP has no Brotli decoder, so the client checks two things it can
...
$client = spawn(function () use ($port, $server, $head, $tail) {
    ...
    [$h, $early, $whole] = tas_flush_probe($port, 'br');
