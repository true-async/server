...
- **`sendFile()` and `StaticHandler` refused every Windows path, and let the whole drive through as a document root (#278).** Both decided "absolute" by looking for a leading slash — `http_response.c:1921` and `http_send_file.c:143` — so on Windows every `sendFile()` answered `500 sendFile(): path must be absolute` and the entire `sendfile` group failed. `StaticHandler` used `IS_ABSOLUTE_PATH`, which is the other half of the same blind spot: it demands a drive letter, so `/x` was refused before the code could say what was really wrong with it. Worse, its volume-root guard compared the resolved root against `"/"`, and a Windows volume root resolves to `C:\`, so `new StaticHandler('/x/', '/')` was accepted and would have served the whole drive. All three sites now reject what they mean to reject — a path relative to the current directory — and the guard recognises a volume root. Evidence: the Windows suite goes from 147 passed / 17 failed to 153 / 10, with `sendfile` fully green.
