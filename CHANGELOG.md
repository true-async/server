...
- **A client still uploading when its body was refused read nothing back (#287).** The 413 never reached the peer, though `parse_errors_413_total` counted it. After a parse error `http_connection_handle_read_completion` returns early and consumes nothing, so `read_buffer_len` stays where the error left it and grows with every chunk the peer goes on sending; `http_connection_alloc_cb` then hands the reactor a zero-length buffer and libuv reports `UV_ENOBUFS`, which latches `write_failed` and takes the connection down before the refusal is delivered. Waiting behind it, a socket closed with unread bytes in its receive buffer is reset rather than finished, and the reset discards what this side has written. Latching a parse error now opens a lingering close — nginx's `lingering_close`, Apache's `ap_lingering_close`: arriving bytes are thrown away instead of buffered, and the destroy waits for 500 ms of silence from the peer, refreshed by every chunk, bounded by 5 s in total. Both bounds reach the connection through the deadline tick that already walks the connection list, so no timer is added and the tick period is the resolution: the wait is at least this long, not exactly. Evidence: `h1/005` fails 3 of 4 runs on Windows before and passes 5 of 5 after; with the upload cut to exactly what trips the limit, leaving nothing inbound, it passes either way, which is the cause.
- **`sendFile()` and `StaticHandler` refused every Windows path, and let the whole drive through as a document root (#278).** Both decided "absolute" by looking for a leading slash — `http_response.c:1921` and `http_send_file.c:143` — so on Windows every `sendFile()` answered `500 sendFile(): path must be absolute` and the entire `sendfile` group failed. `StaticHandler` used `IS_ABSOLUTE_PATH`, which is the other half of the same blind spot: it demands a drive letter, so `/x` was refused before the code could say what was really wrong with it. Its volume-root guard, meanwhile, recognised only `"/"`. That guard is not about taste: the containment check compares `canonical[root_len]` against a separator, and for a volume root that position already holds the first character of the subpath, so every request under such a mount would 404 — a mount that looks configured and answers nothing. Windows spells a whole volume three ways (`C:\`, the device form `\\.\C:\`, and a UNC share root such as `\\host\C$\`), and the last two survive canonicalisation unchanged, so for those the containment prefix does match and the drive really would be served. All three are refused now, and the message names what was wrong instead of quoting a slash the operator never typed. Evidence: the Windows suite goes from 147 passed / 17 failed to 153 / 10, with `sendfile` fully green.
- **A restarted server leaked its previous scope.** `start()` mints a server scope and stores it, but the release lives in `http_server_free`, so a second `start()` on the same object overwrote the field and stranded the first — 56 bytes per restart, and nothing left holding a reference to free them. Invisible until #275 made a restart on the same port possible at all; the macOS debug job reports it as `Total 1 memory leaks detected` at `ext/async/scope.c:1311` against `core/075`. The previous scope object is now released before the new one is created.
- **Several threads could not serve one port: on macOS and the BSDs only the built-in worker pool shared its listening socket, and on Windows nothing shared at all (#275).** `http_server_prebind_tcp` filled a table that only the pool's own start filled in, so a caller spawning its own threads over one transferred server fell through to a private bind per thread and the second `listen()` answered `EADDRINUSE`. Windows had neither strategy: `http_server_use_shared_listen_fd()` and `http_server_use_reuseport()` both returned false unconditionally there and the adoption branch was compiled out, so even the built-in pool could not run `workers > 1`. The listening sockets now live in one refcounted set per server, bound lazily under a mutex by whichever thread reaches `start()` first; every other thread adopts a duplicate of that socket — `dup()` on POSIX, `WSADuplicateSocketW` on Windows, mandatory either way because the reactor closes what it adopts. The set is carried across threads by reference, and its last reference closes the sockets, so a server that never reached `stop()` no longer holds its ports until the process exits. Evidence: `core/006` and `core/007` fail with `address already in use` on a Windows build of #273 and pass here; the Windows suite goes from 139 passed / 22 failed to 142 / 16.
- **A listener that failed to bind left the server in its accept loop with nothing bound, and said nothing.** `libuv_listen_start` throws and returns false when `uv_listen` fails, but both call sites in `http_server_class.c` dropped the answer and carried on; `start()` never returned and no exception reached PHP. On Windows an address conflict surfaces at `listen()` rather than `bind()`, so this is the ordinary path there, not a corner. The result is now checked for both the TCP and the unix listener, the partially built listener set is released, and the failure arrives as `Failed to start listening: address already in use`.
- **`thread_queue.cc` did not compile under MSVC: `#pragma GCC` is an unknown pragma there and `/WX` made it fatal.** The file quiets the vendored moodycamel headers with four `#pragma GCC diagnostic ignored` lines; MSVC answered C4068 six times and C2220 stopped the build at `src/core/thread_queue.cc(42)`. It had never been caught because no source of this extension was compiled on Windows at all. Our own pragmas are now guarded on `__GNUC__`/`__clang__`, and `/wd4068` covers `deps/concurrentqueue/concurrentqueue.h:38`, which carries the same pragma unguarded and is not ours to edit.
- **Neither extension reached the Windows build: the staging checkouts stayed in the php-src root, so every `ARG_ENABLE` was registered twice and the duplicate switched both back off.** `buildconf.js:find_config_w32(".")` scans every directory in the php-src root for a `config.w32`, not only TSRM and Zend, so `async-src` and `http-server-src` each registered as a module beside the copy under `ext/`. `conf_process_args` accepted `--enable-async` and `--enable-true-async-server` on the first entry of each pair, then the loop that applies defaults to arguments the command line did not name reached the second and assigned `PHP_ASYNC = "no"` and `PHP_TRUE_ASYNC_SERVER = "no"`. 735 sources compiled and not one belonged to either extension; configure printed no error and listed neither in its table, and the suite reported 491 tests with 0 executed. `--enable-snapshot-build` had been masking this in the ext/async recipe, not enabling anything: its snapshot pass rewrites every unnamed `'no'` default to `yes,shared` and so switched the extension back on. The staging checkouts are now removed before `buildconf.bat` runs. Reported as true-async/php-async#270 and fixed there in true-async/php-async#271.
- **The Windows build did not link: `config.w32` was missing the framing module and the whole of `src/room/`.** `http_response_framing.c` carries `http_response_length_action_for` and `http_response_header_allowed_h2h3`, which every transport calls, and rooms became unconditional when the pub/sub core stopped knowing what a connection is — `HttpServer::publish()`, `::send()`, `::trySend()`, `::room()`, `::subscriberCount()` and `::enableRooms()` are declared whatever the WebSocket setting is. 18 unresolved externals, and it reached a release because the extension's own Windows job builds none of it: `configure.bat --enable-true-async-server` leaves every source uncompiled there and the phpt run reports 491 tests with 0 executed, so the job is green on an absent extension. WebSocket is still absent from the Windows build, as before, and its call sites are guarded. The Windows test step now asks `php.exe -m` for the module before it trusts the suite, and refuses a run where nothing executed.
