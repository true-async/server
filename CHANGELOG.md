...
- **A streamed body can be framed by a length the handler declares.** A `Content-Length` set before the first `write()` now reaches the client on every protocol and frames the body: HTTP/1 sends it with no `Transfer-Encoding`, no chunk size lines and no terminator, HTTP/2 and HTTP/3 carry the field beside the DATA frames, and the pool wire stops stripping it on a streaming head. Before this the header was dropped on all four paths — chunked encoding on HTTP/1, "implicit from DATA frames" everywhere else — so a client that wanted to size a download, show progress, or check the transfer against a promise had nothing to read. The server holds the body to the number: a write that would pass it throws `HttpServerRuntimeException` and queues nothing, so the handler can catch and still finish what it promised, and a body that ends short is failed the way `abort()` fails one rather than ended cleanly under a header that lies. Such a response is not compressed, since a codec puts a different count on the wire — which is why it deletes the header when it engages. A cancellation is no longer an exemption here: an undeclared feed cut by a graceful shutdown has produced every byte it promised, a declared one has not. A value that is not a decimal byte count, or one set twice through `addHeader()`, throws from that first `write()` while the response is still uncommitted, so it can still become a status instead of being dropped in silence. A response declares at its first streaming call, counted by the bytes that call puts on the wire; three declare nothing — a gRPC call, whose body is closed by a trailer frame the handler never sees, an SSE stream, and a `HEAD`, which keeps the length on the buffered path where it describes the body a `GET` would return. A status that carries no content has no body to frame at all (#197). Evidence: `h1/037` reads nine body bytes with no framing around them and completes a second request on the same connection, `h1/038` and `h1/039` pin the two failure modes, `h2/054` and `h3/060` read the field off the wire and get a reset on a short body, `h3/061` carries it across the pool wire, and `compression/074` shows the same payload gzipped without a declaration and identity with one.
- **A streaming handler can offer a chunk without waiting for room (#177).** `HttpResponse::tryWrite()` returns false when the outbound queue is full, having queued nothing, so the same chunk can be offered again; a client that has gone still throws `HttpException` 499, because "wait" and "stop" need opposite reactions and one bool cannot carry both. The pair mirrors `WebSocket::send()`/`trySend()` and shares their high-water mark, `HttpServerConfig::setStreamWriteBufferBytes()`. The transport answers where the chunk is queued rather than through a predicate read beforehand: `append_chunk` gained a `nonblocking` argument, and a transport that would have parked returns `HTTP_STREAM_APPEND_BACKPRESSURE` instead. HTTP/1 is the exception in both halves — it keeps no queue of its own, so it never refuses and an accepted chunk waits for the socket as `write()` does.
...
- **The two dialects got the non-blocking half as well (#178).** `HttpResponse::trySseEvent()` and `tryWriteMessage()` frame exactly what `sseEvent()` and `writeMessage()` frame and answer false on a full queue, having queued nothing and committed no header, so the same event or message can be offered again. Without them the idiom was half-applied: a handler streaming SSE or gRPC had `tryWrite()` in the API and no way to use it, because the framing lives inside the dialect call. Their arrival also fixed four defects the review found. `awaitWritable()` refused to run in SSE mode, which left a handler that had just been refused by `trySseEvent()` with nowhere to wait; waiting emits no bytes, so that guard and the buffered-body one now apply to the calls that emit. `writeMessage()` answered to neither: a gRPC frame could be pushed into an SSE body, or discard a body set with `setBody()`, and both are now refused as they are for `write()`. `trySseEvent()` probed the peer only after starting the stream, so its 499 arrived on a committed response and a handler could not answer with a status; it probes first. And `awaitWritable($timeoutMs)` documented a deadline no transport read — HTTP/2 now waits the shorter of the argument and the connection's write timeout. HTTP/1 keeps its exception here too: no queue, so no refusal.
- **A streaming response can be disowned instead of finished (#171).** `HttpResponse::abort()` ends a started stream as failed. Until now a handler that threw halfway still had its body sealed by the dispose path — HTTP/1 wrote the terminating chunk, HTTP/2 and HTTP/3 ended the stream normally — so a CSV export that hit a database error after 5 of 10 MB answered `200 OK` with half the file and a clean end, and `curl -o` exited 0 on a file the application never meant to produce. There was no combination of the old API that produced anything else. Each protocol now says it the one way it can: HTTP/1 withholds the terminator and drops the connection (`curl` reports `CURLE_PARTIAL_FILE`), HTTP/2 sends `RST_STREAM(INTERNAL_ERROR)`, HTTP/3 resets the stream, and a pool worker posts an abort its reactor turns into the same reset. `abort($errorCode)` names the code instead — the reset code of whichever protocol carries the response, which is why it is not portable between them and why HTTP/1 ignores it. An uncaught exception takes that route by itself; a cancellation does not, so a graceful shutdown still ends the feeds it interrupts cleanly. `abort()` is a no-op on a response that never started streaming and on a second call — its place is a catch block, where a method that throws buries the handler's own error — and refuses only after `end()`, which has already told the client the body is whole. Evidence: `h1/033` reads the wire and finds the frames but no terminator, `h2/053` reads error code 2 off the RST frame and completes a second request on the same connection, `h3/058` and `h3/059` get `RESET(err=258)` out of an independent QUIC stack, and `grpc/018` proves a gRPC call still ends with `grpc-status: 13` rather than a reset.
...
- **BC: `isEnded()` answers true after `abort()` as well as after `end()` (#171).** It reports that the response is finished, not which call finished it, so a loop written as `while (!$res->isEnded())` stops on a failed stream instead of writing into one. A handler that needs to tell the two apart has the exception that caused the abort.
...
- **BC: `setStatusCode()` takes 200 to 599, and refuses an interim status (#197).** A 1xx used to be accepted and framed as a complete message. It is not one: RFC 9110 §15.2 makes it interim, so a client that received it read the header block and went on waiting for the final response — which the handler had no way to send, because the request is finished when it returns. Both ends then waited for the other until a timeout, and a 1.0 client, which §15.2 forbids a 1xx to entirely, read it as the answer. `setStatusCode(100)` now throws `HttpServerInvalidArgumentException`. The server has no interim-response API; a handler that wants to acknowledge early has `write()`, which sends a real response.
...
- **HTTP/2 and HTTP/3 sent no `Content-Length` on any response, `HEAD` and static files included (#200).** The header filter dropped the name on every path, on the reading that DATA frames make the count implicit, and nothing on the response side of either protocol computed one to put back — so a `HEAD` over HTTP/2 answered with no size at all, and a 4 KiB file served by `sendFile()` reached the client with nothing to size the download by. RFC 9110 §8.6 asks for the field wherever the size is known before the header section is sent and draws no line between protocol versions; §9.3.2 wants a `HEAD` response to carry the fields its `GET` would. The count is now the server's on all three transports: a buffered response states the length of the buffer it is about to send, `sendFile()` and the static engine state the file size or the range length, and a handler value that disagrees is replaced rather than forwarded, since RFC 9113 §8.1.1 makes a mismatch a malformed message that nghttp2 answers with a stream error. Nothing is stated where nothing is known — an undeclared stream, a `HEAD` whose handler streamed — and a 1xx, a 204 and a response carrying trailers send no field, the last because nghttp2 ends the stream at the byte completing the count and the trailing HEADERS would reach it closed. The rules HTTP/1 has held since #195 and #197 moved out of `http1_format.c` and now answer for all three. `static/012` reads `cl=4096` on `GET` and on `HEAD` where it read `cl=-`, and `h3/019` counts 72 response headers where it counted 71.
- **A streaming response to an HTTP/1.0 client was framed by chunked coding it cannot read (#197).** `Transfer-Encoding: chunked` went out on every streamed response with no declared length, whatever version the request used, while the status line answered `HTTP/1.0` — so a 1.0 client, which has no chunked decoder and never asked for one, read `5\r\nalpha\r\n4\r\nbeta\r\n0\r\n\r\n` as twenty-six body bytes where nine were sent. RFC 9112 §6.1 forbids sending the header to such a client. Such a body is now delimited by the connection close (§6.3 rule 7): no framing header, the bytes as they are, `Connection: close` in the header block, and the connection ends with the response — so a request pipelined behind it goes unanswered, as one behind a broken chunked body already did. A 1.0 client that asked for `Connection: keep-alive` and gets a declared `Content-Length` keeps its connection instead, and the server now echoes `Connection: keep-alive` back, which such a client needs before it will reuse a socket; a 1.0 request that did not ask still gets the close, declared length or not. A streaming response also never advertised `Connection: close` at all: the header was set in the dispose path, long after the header block had left at the first `write()`, so a request that asked for close and a graceful drain that wanted the connection retired both went unmentioned to every streaming client. HTTP/1.1 framing is unchanged.
- **A status that carries no body was sent one (#197).** `1xx`, `204` and `304` end at the header block whatever the header fields say (RFC 9112 §6.3 rule 1), and the server ignored that on both paths: `setStatusCode(204)->setBody('oops')` sent `Content-Length: 4` and four bytes, so the next response on the connection was read from inside them, and a streaming call on the same status added `Transfer-Encoding: chunked` and a chunked body. An empty `204` still carried `Content-Length: 0`, which RFC 9110 §8.6 forbids. The two body modes now answer differently, and the difference is where the handler can still act: a streaming call throws `HttpServerRuntimeException` while the response is uncommitted, so it can answer with a status it means, and a buffered body is dropped at format time, because the status may legitimately have been chosen after the body was built — a conditional `GET` renders a representation and then finds the `ETag` matches. `sseStart()` refuses such a status for the same reason. A `304` keeps a handler `Content-Length`, which describes the representation a `200` would have carried (§8.6); a `1xx` and a `204` lose it. The refusal lives in the streaming call every transport shares, so it applies on HTTP/2 and HTTP/3 as well, where RFC 9110 §6.4.1 puts the same statuses among those defined to carry no content. A `HEAD` response was shipping a body through `sseEvent()` and `writeMessage()`, which never had the drop `write()` and `tryWrite()` do; it does now.
- **The buffered path sent `Content-Length` beside a handler `Transfer-Encoding` (#197).** A handler that set the header got it copied onto the wire next to the server's own count — `Content-Length: 5` and `transfer-encoding: chunked` on the same response. RFC 9112 §6.1 forbids the pair and §6.3 names it as the shape one intermediary resolves by the length and the next by the chunks, which is how a request is smuggled past the first. The streaming path stopped emitting the pair with the declared-length work; the buffered path drops the handler's `Transfer-Encoding` now too, because on both the framing is the server's to state.
- **An exception message reached the HTTP/1 status line unfiltered, so a CRLF in it split the response (#198).** An uncaught handler exception's message becomes the reason phrase of the synthesised 500, and `setReasonPhrase()` took whatever it was given, so `throw new RuntimeException("bad path: $path")` put request data between the status code and the CRLF that ends the status line. A message containing `\r\n` ended that line early: everything behind it was read as header fields and, past a blank line, as a second response the server never sent — one 500 became a 500 with an injected header plus a 200 with a body (CWE-113). The phrase now keeps only what RFC 9112 §4 allows there — HTAB, SP, VCHAR and obs-text — and every other byte becomes a space, which leaves a UTF-8 message readable and nothing that can end a line. The body is untouched: `Content-Length` frames it, so it still carries the message as written. HTTP/2 and HTTP/3 have no reason phrase and were never affected.
- **`sendFile()` and `json()` were two more doors onto the status code, and wider than the front one (#197).** `setStatusCode()` refuses an interim status, and both of these wrote `status_code` past it: `json($data, 100)` produced the interim response that hangs the exchange, and `new SendFileOptions(status: 204)` framed a whole file under a status the client ends at the blank line — it then read the file's first bytes as the next status line, which is the desync `core/065` pins on the PHP path, reachable through a second door in the same release. All three callers ask one predicate now, and `sendFile()` additionally refuses a status defined to carry no content, because a file body is content. Evidence: `sendfile/004`.
- **A 205 Reset Content carried a body, and a body it could not frame (#197).** `setStatusCode(205)->setBody('x')` sent the bytes, and a streaming call on the same status sent chunks. RFC 9110 §15.3.6 forbids content in a 205. It is not the same case as a 204: RFC 9112 §6.3 rule 1 does not name 205, so the client still looks for framing, and a 205 with no `Content-Length` leaves it reading the next response as this one's content. The body is dropped on both paths, and the emptiness is stated — `Content-Length: 0` — rather than left to be inferred. `http_status_reason` gained the phrase as well; a 205 answered `205 Unknown` before. Evidence: `core/065`, which pipelines a second request behind the 205 and reads its status line where the 205 ended.
- **A `HEAD` whose handler streamed reported `Content-Length: 0` (#197).** `write()` dropped the chunk and returned before the stream committed, so the response fell through to the buffered path and had its length computed from a buffer nobody had filled — which states that the `GET` body is empty, the opposite of what RFC 9110 §9.3.2 wants the field to mean. The response commits as a streaming one now, dropping only the bytes, so it carries the framing the same handler's `GET` would carry and states no length it never measured; a `Content-Length` the handler declared itself survives and describes the body a `GET` would return. The four streaming calls and the two SSE ones now agree on this, where before `sseEvent()` committed and `write()` did not. Evidence: `core/065`, routes `/streamhead` and `/streamheadbare`.
- **A handler `Connection: close` was copied to the client and ignored by the server (#197).** `setHeader('Connection', 'close')` on an HTTP/1.1 response put the field on the wire while `conn->keep_alive` stayed true, so the server answered the next request on a socket the client had already retired — through a pooling proxy, on a request that pool no longer associates with that connection. On a 1.0 response the keep-alive echo overwrote the handler's value with `keep-alive`, which is the same mistake in the other direction: obeyed where obeying it desyncs, overruled where it was right. The field is now read rather than copied: `close` retires the connection after the response on HTTP/1, and the peer is told by the same branch that tells it about a drain. `keep-alive` is dropped, being what the server was going to say anyway — refusing it would turn a correct response into a 500 for a handler copying an upstream response's headers — and any other value throws. HTTP/2 and HTTP/3 multiplex, so one response never retires their connection and the request is recorded and unused there. In the same pass `Transfer-Encoding` stopped being dropped whatever it named: `chunked` is accepted and dropped, since it is the framing an undeclared HTTP/1.1 stream gets anyway, and anything else throws — a handler that set `gzip` had the field removed and the encoded bytes sent with no coding declared, which is a corrupt download rather than a header worth second-guessing. Evidence: `h1/047`, which reads the wire and then writes a second request onto the same socket.
- **A header name or value reached the HTTP/1 wire unchecked, so a CRLF in one split the response (#198).** `setHeader('Location', $next)` copied whatever the handler passed between the field name and the CRLF that ends the line, and `redirect()` did the same with its URL. A value containing `\r\n` ended the header block early: the bytes behind it were read as further fields and, past a blank line, as a second response the server never sent (CWE-113). This is the same defect the reason phrase carried, on the route request data actually takes — a redirect target, a request id echoed back. `setHeader()`, `addHeader()`, `redirect()` and `setTrailer()` now throw `HttpServerInvalidArgumentException` naming the offending byte and its offset: a name must be an RFC 9110 §5.6.2 token, and a value may carry only what §5.5 allows — HTAB, SP, VCHAR and obs-text — with no leading or trailing whitespace, which the same section forbids a sender to generate. The bytes checked are the bytes stored, so a value handed over as an object is checked after its `__toString()`; a PSR-7 URI built from a query parameter is the commonest way request data reaches `Location`. Refused rather than cleaned, unlike the phrase, because these calls run while the handler can still answer with something else; a refusal stores nothing, and `redirect()` checks its URL before it assigns the status, so a refused call leaves the response as it was. HTTP/2 and HTTP/3 were covered by nghttp2's and nghttp3's own validators. Evidence: `h1/046`.
- **A handler `Content-Length` that disagreed with a buffered body desynced the connection.** On HTTP/1 a handler value was emitted verbatim next to whatever body the response actually held: `setHeader('Content-Length', '5')` with an eleven-byte body sent all eleven under a header naming five, and on a keep-alive connection the client read the remaining six as the status line of the next response. The buffered path was the only one that ever passed a handler value through, and it compared it with nothing. The server now states the count it is sending; the handler's number is kept only on `HEAD`, where the body is suppressed by the method and the value describes the body a `GET` would return.
...
- **A compressed stream whose handler forgot `end()` was undecodable (#171).** Two finishers disagreed about where a stream ends. `end()` went through the compressing wrapper, which closes the codec stream and emits its trailer; falling out of the handler instead reached the transport underneath and wrote the chunked terminator by hand, so the client got a body that was complete by every framing rule it could check and undecodable by the only one that mattered — `gzdecode()` refused it and 1800 bytes of a 1800-byte body were lost. Every dispose path now finishes through the response's own ops rather than the transport's, on all four transports, which is also what keeps the abort from writing that trailer over half a body. Test `compression/071` decodes the whole body out of a handler that never calls `end()`; `compression/072` is the other half, where the decoder must refuse.
- **A stream reset by the server could be reported as reset by the peer.** nghttp2 delivers a locally sent RST_STREAM through the same close callback as a remote one, so a handler that called `abort()` had its own error replaced by `HttpException` 499 "stream reset by peer", and the reset was counted in the peer-reset metric. Both now exclude a reset the server sent itself — keyed on the latch rather than on which coroutine is running, because the send that carries the reset can be deferred behind an in-flight write.
- **A pipelined HTTP/1 request behind a broken stream was still answered (#171).** The connection answers a request already in its read buffer before it honours a close decision, so that a client gets a reply to everything it sent — and that stops being right once a chunked body has ended without its terminator. The peer is still counting down the bytes an unfinished chunk promised it, so the next response arrived as that chunk's data: a whole `HTTP/1.1 200 OK` read as body. It now goes unanswered, on an aborted stream and on the older case of a cancellation cutting a frame in half.
- **A pool worker's abort could still reach the client as a clean end.** The reactor applies the wires of one response in a single tick and flushes at the end of it, so raising the stream's end flag before that flush let the data reader emit EOF and the reset arrive after the peer had already been told the body was whole — observed in one run out of three. The pending bytes are drained before the flag goes up, which also means the peer keeps the part of the body the handler did produce instead of losing it to the reset.
