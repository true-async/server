#!/usr/bin/env bash
# Smoke test for the multi-worker TrueAsync HTTP server.
#
#   ./examples/smoke-test.sh                # auto workers = available_parallelism()
#   WORKERS=8 PORT=9090 ./examples/smoke-test.sh
#
# Starts examples/multi-worker.php in the background, hits a handful of
# endpoints, runs wrk if installed, then stops the server.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
EXT="$ROOT/modules/true_async_server.so"
ENTRY="$ROOT/examples/multi-worker.php"

PORT="${PORT:-18080}"
TLS_PORT="${TLS_PORT:-18443}"
H3_PORT="${H3_PORT:-$TLS_PORT}"
TLS_CERT="${TLS_CERT:-$ROOT/examples/certs/server.crt}"
TLS_KEY="${TLS_KEY:-$ROOT/examples/certs/server.key}"
WORKERS="${WORKERS:-0}"   # 0 => auto via available_parallelism()
H3CLIENT="$ROOT/tests/h3client/h3client"
LOG="$(mktemp -t tas-mw.XXXXXX.log)"

[[ -f "$EXT" ]]   || { echo "missing extension: $EXT" >&2; exit 1; }
[[ -f "$ENTRY" ]] || { echo "missing entry: $ENTRY"   >&2; exit 1; }

# Make sure the port isn't held by a stale run.
...
echo ">> starting WORKERS=$WORKERS (0 = auto) PORT=$PORT TLS_PORT=$TLS_PORT TLS=$TLS_AVAILABLE"
WORKERS="$WORKERS" PORT="$PORT" TLS_PORT="$TLS_PORT" H3_PORT="$H3_PORT" \
TLS_CERT="$TLS_CERT" TLS_KEY="$TLS_KEY" \
    php -d extension="$EXT" "$ENTRY" >"$LOG" 2>&1 &
SERVER_PID=$!
cleanup() {
...
# Surface CPU diagnostics from the server log so the user sees what was picked.
...
JSON_BODY="$(curl -fsS "http://127.0.0.1:$PORT/json/3")"
[[ "$JSON_BODY" == *'"count":3'* && "$JSON_BODY" == *'"items"'* ]] \
    && pass "GET /json/3 shape" \
    || fail "GET /json/3 shape" "$JSON_BODY"

# n is clamped to the dataset size (64 in multi-worker.php).
JSON_BIG="$(curl -fsS "http://127.0.0.1:$PORT/json/9999")"
[[ "$JSON_BIG" == *'"count":64'* ]] \
    && pass "GET /json/9999 clamped to 64" \
    || fail "json clamp" "$JSON_BIG"
...
# Keep-alive: 5 requests on one curl invocation; only the first should open a
# new TCP connection, so the sum of num_connects across 5 transfers must be 1.
...
# Pipelining: 4 requests batched on one connection.
...
# Multi-worker dispatch: per-worker static $count differs across threads.
echo ">> multi-worker dispatch check"
...
[[ "$DISTINCT" -ge 2 ]] \
    ...
    || fail "multi-worker" "only $DISTINCT distinct counter(s) — REUSEPORT not balancing?"
...
if [[ "$TLS_AVAILABLE" == 1 ]]; then
    ...
    # Alt-Svc must advertise h3 so clients can auto-upgrade.
    ...
    # HTTP/3 — only if our h3client is built.
    if [[ -x "$H3CLIENT" ]]; then
        ...
        [[ "$H3_JSON" == *'"count":3'* ]] \
            && pass "HTTP/3 GET /json/3" \
            || fail "h3 json" "$H3_JSON"
    else
        echo "  SKIP  HTTP/3 (h3client not built — see tests/h3client/h3client.c)"
    fi
fi

if command -v wrk >/dev/null; then
    echo ">> 5s load test (wrk -t4 -c64)"
