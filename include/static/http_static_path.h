...
/* Longest hide pattern the matcher reads, separators included. StaticHandler::hide
 * refuses a longer one rather than letting it match nothing. */
 ...
 * drag its contents in — `cache/` says that instead. Takes the two strings
 * rather than the mount so the rule can be held to a table. Both must be
 * NUL-terminated; a pattern longer than HTTP_STATIC_HIDE_GLOB_MAX covers
 * nothing. */
