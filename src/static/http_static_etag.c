...
void http_static_etag_format(const struct stat *st, char buf[HTTP_STATIC_ETAG_BUF_LEN])
{
	...
	buf[19] = '"';
	buf[20] = '\0';
	ZEND_STATIC_ASSERT(HTTP_STATIC_ETAG_LEN == 20, "etag length must match hand-format");
}
...
void http_static_format_http_date(time_t t, char buf[HTTP_STATIC_DATE_BUF_LEN])
{
	...
	buf[28] = 'T';
	buf[29] = '\0';
	ZEND_STATIC_ASSERT(HTTP_STATIC_DATE_LEN == 29, "date length must match hand-format");
}
...
static bool match_if_none_match(const char *header, size_t header_len, const char *etag,
								size_t etag_len)
{
