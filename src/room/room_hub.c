...
#define ROOM_HUB_MAILBOX_CAPACITY 4096
#define ROOM_HUB_MAILBOX_BATCH      64

/* Interest filter (room_hub.h). Power of two — the probe sequence masks. 1024
...
#define ROOM_INTEREST_BUCKETS 1024u
#define ROOM_INTEREST_PROBES     3u
...
struct room_hub_s {
    ...
    zend_atomic_int64 next_receiver_id;
    zend_atomic_int64 posted;
    zend_atomic_int64 skipped;
    zend_atomic_int64 dropped;

    /* Reliable-send (room_hub_send / room_hub_try_send) counters. Kept apart
    ...
    thread_mailbox_t *inbox[ROOM_HUB_MAX_WORKERS];
     ...
     * thread's query. */
    uint32_t          gen[ROOM_HUB_MAX_WORKERS];
    bool              taken[ROOM_HUB_MAX_WORKERS];
    ...
    zend_atomic_int  *interest[ROOM_HUB_MAX_WORKERS];
 ...
 * the inbox WITHOUT bumping gen, so the drain checks both (see room_hub_retry_drain). */
typedef struct {
    int      slot;
    uint32_t gen;
} retry_target_t;
 ...
 * cloned from room_query_t (same early-resume / cancel hazards): a blocking send()
 ...
 *
 * The topic rides inline because each retry rebuilds a fresh room_cmd_t (the topic
 ...
 * thread's room_local_t (via room_local_of) — never a raw room_local_t*, which detach
 * may have freed. */
typedef struct {
 ...
 * the walk is free. */
ZEND_TLS room_local_t *room_locals = NULL;

static room_local_t *room_local_of(const room_hub_t *hub)
{
    for (room_local_t *local = room_locals; local != NULL; local = local->next) {
        if (local->hub == hub) {
            return local;
        }
    }
...
static uint64_t room_atomic_u64_add(zend_atomic_int64 *counter, const int64_t delta)
{
...
static void room_hub_note_drop(room_hub_t *hub)
{
    (void) room_atomic_u64_add(&hub->dropped, 1);
}
                                        ...
                                        const bool binary)
{
    ...
    room_payload_t *const payload = pemalloc(sizeof(*payload) + len, 1);
    payload->hub    = hub;
...
static void room_payload_addref(room_payload_t *payload)
{
    zend_atomic_int_fetch_add(&payload->refcount, 1);
}

static void room_payload_release(room_payload_t *payload)
{
    if (zend_atomic_int_fetch_add(&payload->refcount, -1) == 1) {
        if (payload->hub != NULL) {
            (void) room_atomic_u64_add(&payload->hub->bodies_freed, 1);
        }
...
static void room_query_release(room_query_t *query)
{
...
room_hub_t *room_hub_create(const uint32_t retry_interval_ms)
{
    room_hub_t *const hub = pecalloc(1, sizeof(*hub), 1);

    hub->retry_interval_ms = retry_interval_ms != 0 ? retry_interval_ms : 50u;

    ZEND_ATOMIC_INT64_INIT(&hub->next_receiver_id, 1);
 ...
 * tree's except_id rests on (room_tree.h). Assigned on the first subscribe,
 * because nothing else about a receiver needs one. */
static void room_hub_assign_id(room_hub_t *hub, room_receiver_t *receiver)
{
    if (receiver->id == 0) {
        receiver->id = room_atomic_u64_add(&hub->next_receiver_id, 1);
    }
}
...
static room_tree_t *room_hub_local_tree(const room_hub_t *hub)
{
                                                        ...
                                                        const uint32_t max)
{
    ...
    if (tree == NULL) {
        return ROOM_HUB_SUBSCRIBE_DETACHED;
    }

    room_hub_assign_id(hub, receiver);

    return room_topic_subscribe(tree, receiver, filter, max)
        ? ROOM_HUB_SUBSCRIBE_OK
        : ROOM_HUB_SUBSCRIBE_AT_CAP;
}
...
{
    room_topic_unsubscribe_all(room_hub_local_tree(hub), receiver);
}
                                ...
                                uint32_t bucket[ROOM_INTEREST_PROBES])
{
    ...
    for (uint32_t i = 0; i < ROOM_INTEREST_PROBES; i++) {
        bucket[i] = (h1 + i * h2) & (ROOM_INTEREST_BUCKETS - 1u);
    }
}
                               ...
                               const size_t prefix_len, const int delta)
{
    ...
    uint32_t bucket[ROOM_INTEREST_PROBES];
    room_interest_probe(filter, prefix_len, bucket);
...
void room_hub_interest_add(room_hub_t *hub, const char *filter, const size_t prefix_len)
{
    room_interest_bump(hub, filter, prefix_len, 1);
}

void room_hub_interest_remove(room_hub_t *hub, const char *filter, const size_t prefix_len)
{
    room_interest_bump(hub, filter, prefix_len, -1);
}
...
typedef struct {
    uint32_t bucket[ROOM_TOPIC_MAX_PREFIXES][ROOM_INTEREST_PROBES];
                                ...
                                const size_t topic_len)
{
    ...
    if (!room_topic_prefixes(topic, topic_len, &prefixes)) {
        interest->count = 0;
        return;
    }

    for (uint32_t i = 0; i < prefixes.count; i++) {
        room_interest_probe(topic, prefixes.len[i], interest->bucket[i]);
    }
                                  ...
                                  const room_interest_t *interest)
{
    ...
    for (uint32_t p = 0; p < interest->count; p++) {
        ...
        for (uint32_t i = 0; i < ROOM_INTEREST_PROBES && hit; i++) {
 ...
 * post runs through room_hub_post_locked holding the lock); the setter below writes
...
static bool room_hub_post_locked(room_hub_t *hub, const int slot, room_cmd_t *cmd)
{
#ifdef TAS_TEST_HOOKS
...
static void room_hub_drain(void **items, const size_t count, void *arg);

/* ------------------------------------------------------ server subscribers
 *
 * A subscriber that queues the payload the tree hands it and wakes the coroutine
 * parked in recv(). Thread-local like the rest of room_local_t: created, drained
...
#define ROOM_SERVER_SUB_RING 64
                                    ...
                                    const size_t len, const bool binary, void **shared)
{
    ...
    room_payload_t *payload = *shared;

    if (payload == NULL) {
        payload = room_payload_new(sub->local != NULL ? sub->local->hub : NULL,
                                 data, len, binary);
        *shared = payload;
    }

    if (sub->size == ROOM_SERVER_SUB_RING) {
        room_payload_release(sub->ring[sub->head]);
        sub->head = (sub->head + 1) % ROOM_SERVER_SUB_RING;
        sub->size--;
        sub->lost++;

        if (sub->local != NULL) {
            (void) room_atomic_u64_add(&sub->local->hub->sub_overflow, 1);
        }
    }

    room_payload_addref(payload);   /* the ring's reference; the walk keeps its own */
    sub->ring[(sub->head + sub->size) % ROOM_SERVER_SUB_RING] = payload;
    sub->size++;
...
static const room_receiver_ops_t room_server_sub_ops = {
    .deliver = room_server_sub_deliver,
};

static void room_server_sub_addref(room_server_sub_t *sub)
{
    sub->refcount++;
}
...
static void room_server_sub_drop_ring(room_server_sub_t *sub)
{
    while (sub->size > 0) {
        room_payload_release(sub->ring[sub->head]);
        sub->head = (sub->head + 1) % ROOM_SERVER_SUB_RING;
...
static void room_server_sub_release(room_server_sub_t *sub)
{
    if (--sub->refcount > 0) {
        return;
    }

    room_server_sub_drop_ring(sub);
...
static void room_hub_retry_teardown(room_hub_t *hub, room_local_t *local, bool request_over);

int room_hub_attach(room_hub_t *hub)
{
    if (hub == NULL || room_local_of(hub) != NULL) {
        return -1;
    }

    thread_mailbox_t *const inbox = thread_mailbox_create(
        ROOM_HUB_MAILBOX_CAPACITY, ROOM_HUB_MAILBOX_BATCH, room_hub_drain, hub);
    ...
    zend_atomic_int *const counters =
        pecalloc(ROOM_INTEREST_BUCKETS, sizeof(*counters), 1);

    for (uint32_t i = 0; i < ROOM_INTEREST_BUCKETS; i++) {
    ...
    for (int i = 0; i < ROOM_HUB_MAX_WORKERS; i++) {
        if (!hub->taken[i]) {
            hub->taken[i]    = true;
            hub->inbox[i]    = inbox;
    ...
    /* Paired with the drop in room_hub_detach: the slot outlives this thread's
     * server object on the failure paths out of start(), so it holds its own. */
    zend_atomic_int_fetch_add(&hub->refcount, 1);

    room_local_t *const local = ecalloc(1, sizeof(*local));
    local->hub   = hub;
    local->slot  = slot;
    local->gen   = gen;
    local->inbox = inbox;
    local->tree  = room_tree_create(hub);
    local->next  = room_locals;
...
static void room_hub_detach_ex(room_hub_t *hub, bool request_over);

void room_hub_thread_sweep(void)
{
    while (room_locals != NULL) {
        room_hub_detach_ex(room_locals->hub, /*request_over*/ true);
    }
}

/* Attaches once per thread; the shutdown paths are the only detach. */
static room_local_t *room_hub_attach_ensure(room_hub_t *hub)
{
    if (hub == NULL) {
        return NULL;
    }

    room_local_t *local = room_local_of(hub);

    if (local != NULL) {
        return local;
    }

    return room_hub_attach(hub) >= 0 ? room_local_of(hub) : NULL;
}

room_server_sub_t *room_hub_subscribe(room_hub_t *hub, zend_string *filter)
{
    room_local_t *const local = room_hub_attach_ensure(hub);

    if (local == NULL) {
        return NULL;
    }

    room_server_sub_t *const sub = ecalloc(1, sizeof(*sub));
    sub->local        = local;
    sub->filter       = zend_string_copy(filter);
    sub->receiver.ops = &room_server_sub_ops;
    sub->refcount     = 1;   /* the tree's side; the caller takes its own below */

    room_hub_assign_id(hub, &sub->receiver);

    /* No quota: a server subscriber is code, not a peer connection to protect
     * from itself. */
    if (!room_topic_subscribe(local->tree, &sub->receiver, filter, 0)) {
        room_server_sub_release(sub);
        return NULL;
    }

    room_server_sub_addref(sub);   /* the caller's side */
...
void room_hub_unsubscribe(room_server_sub_t *sub)
{
    ...
    room_local_t *const local = sub->local;

    sub->closed = true;

    if (local != NULL) {
        room_topic_unsubscribe_all(local->tree, &sub->receiver);

        for (room_server_sub_t **it = &local->subs; *it != NULL; it = &(*it)->next) {
            if (*it == sub) {
                *it = sub->next;
                break;
            }
...
static room_payload_t *room_server_sub_pop(room_server_sub_t *sub)
{
    if (sub->size == 0) {
        return NULL;
    }

    room_payload_t *const payload = sub->ring[sub->head];
    sub->head = (sub->head + 1) % ROOM_SERVER_SUB_RING;
    sub->size--;
 ...
 *
 * NULL on either side means the attachment is being torn down — room_hub_detach
...
static bool room_local_recv_keepalive(room_local_t *local, const bool parked)
{
...
static bool room_park_add_callback(zend_async_event_t *event, zend_async_event_callback_t *callback)
{
    return zend_async_callbacks_push(event, callback);
}

static bool room_park_del_callback(zend_async_event_t *event, zend_async_event_callback_t *callback)
{
    return zend_async_callbacks_remove(event, callback);
}

static bool room_park_start(zend_async_event_t *event)
{
    room_park_event_t *const park = (room_park_event_t *) event;

    if (ZEND_ASYNC_EVENT_IS_CLOSED(event) || park->latched || park->sub == NULL) {
        return true;
    }

    park->latched = room_local_recv_keepalive(park->sub->local, true);

    return true;
}

static bool room_park_stop(zend_async_event_t *event)
{
    room_park_event_t *const park = (room_park_event_t *) event;
    ...
    park->latched = false;
    (void) room_local_recv_keepalive(park->sub->local, false);

    return true;
}

static bool room_park_dispose(zend_async_event_t *event)
{
    room_park_event_t *const park = (room_park_event_t *) event;
    ...
    if (park->sub != NULL) {
         ...
         * `closed`, and room_hub_recv tests `closed` BEFORE `parked`. Swap those
...
static zend_string *room_park_info(zend_async_event_t *event)
{
    const room_park_event_t *const park = (const room_park_event_t *) event;
...
static room_park_event_t *room_park_event_new(room_server_sub_t *sub)
{
    room_park_event_t *const park = pecalloc(1, sizeof(*park), 0);

    park->base.ref_count    = 1;
    park->base.add_callback = room_park_add_callback;
    park->base.del_callback = room_park_del_callback;
    park->base.start        = room_park_start;
    park->base.stop         = room_park_stop;
    park->base.dispose      = room_park_dispose;
    park->base.info         = room_park_info;
                                     ...
                                     room_payload_t **out)
{
    *out = NULL;
    ...
    if (sub->parked) {
        return ROOM_HUB_RECV_BUSY;
    }

    *out = room_server_sub_pop(sub);

    if (*out != NULL) {
        return ROOM_HUB_RECV_MESSAGE;
    }
    ...
    room_server_sub_addref(sub);

    room_park_event_t *const park = room_park_event_new(sub);
    ...
    if (UNEXPECTED(!did_park)) {
         ...
         * one over it. */
        return EG(exception) != NULL ? ROOM_HUB_RECV_CANCELLED : ROOM_HUB_RECV_CLOSED;
    }

    room_hub_recv_status_t status;
    ...
    if (EG(exception) != NULL) {
        ...
        status = ROOM_HUB_RECV_CANCELLED;
    } else if (sub->closed) {
        status = ROOM_HUB_RECV_CLOSED;
    } else {
        *out   = room_server_sub_pop(sub);
        status = *out != NULL ? ROOM_HUB_RECV_MESSAGE : ROOM_HUB_RECV_TIMEOUT;
    }
...
{
    room_payload_release(payload);
}

static void room_local_unlink(room_local_t *prev, const room_local_t *local)
{
    if (prev != NULL) {
        prev->next = local->next;
    } else {
        room_locals = local->next;
    }
}
...
static void room_hub_detach_ex(room_hub_t *hub, const bool request_over)
{
    room_local_t *local = room_locals;
    room_local_t *prev  = NULL;
    ...
    room_hub_retry_teardown(hub, local, request_over);
    ...
    while (local->subs != NULL) {
        room_server_sub_t *const sub = local->subs;
        ...
        room_topic_unsubscribe_all(NULL, &sub->receiver);
        ...
        if (request_over) {
            ...
            sub->waiter = NULL;
            room_server_sub_drop_ring(sub);
            room_server_sub_release(sub);   /* the tree's side */
        } else {
            room_hub_unsubscribe(sub);
        }
    }

    room_tree_free(local->tree);

    efree(local);

    /* Last touch of the hub — after this the owner may already be gone and the
     * block may be freed here. */
    room_hub_release(hub);
}

void room_hub_detach(room_hub_t *hub)
{
    room_hub_detach_ex(hub, /*request_over*/ false);
}

void room_hub_detach_request_over(room_hub_t *hub)
{
    room_hub_detach_ex(hub, /*request_over*/ true);
}
...
static void room_hub_answer_count(room_hub_t *hub, room_cmd_t *cmd)
{
    room_query_t *const query = cmd->query;

    cmd->query = NULL;   /* the reference travels on with the reply */

    const room_local_t *const local = room_local_of(hub);

    room_cmd_t *const reply = room_cmd_new(ROOM_CMD_COUNT_REPLY, NULL, 0);
    reply->query = query;
    reply->count = local != NULL
        ? room_topic_count(local->tree, cmd->topic, cmd->topic_len) : 0;

    tsrm_mutex_lock(hub->admin);

    const bool posted = query->gen == hub->gen[query->slot]
        && room_hub_post_locked(hub, query->slot, reply);
    ...
    if (!posted) {
        room_hub_note_drop(hub);
        room_query_release(query);
    }
}

/* Runs on the thread that started the query — see room_query_t. */
static void room_query_settle(room_query_t *query, const uint32_t answered)
{
    if (!query->abandoned) {
...
static void room_query_orphan(room_query_t *query)
{
    if (!query->abandoned) {
        query->abandoned = true;
        query->done      = NULL;

        room_query_release(query);   /* the asker's side */
    }

    room_query_release(query);       /* this reply's side */
}

static void room_hub_drain(void **items, const size_t count, void *arg)
{
    room_hub_t *const hub = arg;

    const room_local_t *const local = room_local_of(hub);

    for (size_t i = 0; i < count; i++) {
        room_cmd_t *const cmd = items[i];

        switch (cmd->kind) {
            case ROOM_CMD_PUBLISH:
                if (local != NULL) {
                    ...
                    (void) room_topic_publish(local->tree, cmd->topic, cmd->topic_len,
                                            cmd->payload->data, cmd->payload->len,
                                            cmd->payload->binary, cmd->except_id, &shared);
                }

                room_payload_release(cmd->payload);   /* also the walk's reference */
                break;

            case ROOM_CMD_COUNT:
                room_hub_answer_count(hub, cmd);
                break;

            case ROOM_CMD_COUNT_REPLY:
                ...
                if (local != NULL) {
                    room_query_settle(cmd->query, cmd->count);
                } else {
                    room_query_orphan(cmd->query);
                }
 ...
 * unconditionally releases it (room_hub_drain, ROOM_CMD_PUBLISH) — the
...
static uint32_t room_hub_fanout_locked(room_hub_t *hub, const room_local_t *local,
        const char *topic, const size_t topic_len,
        const char *data, const size_t len, const bool binary, const uint64_t except_id,
        const room_interest_t *interest, room_payload_t **payload_io,
        ...
        uint64_t *skipped_out, uint64_t *dropped_out, uint32_t *workers_out)
{
    ...
    for (int slot = 0; slot < hub->slots_used; slot++) {
        ...
        if (!room_interest_matches(hub, slot, interest)) {
            skipped++;
            continue;
        }

        if (payload == NULL) {
            payload = room_payload_new(hub, data, len, binary);
        }

        room_cmd_t *const cmd = room_cmd_new(ROOM_CMD_PUBLISH, topic, topic_len);
        cmd->payload   = payload;
        cmd->except_id = except_id;

        zend_atomic_int_fetch_add(&payload->refcount, 1);

        if (room_hub_post_locked(hub, slot, cmd)) {
            posted++;
        } else {
            room_payload_release(payload);
            pefree(cmd, 1);
...
{
    room_hub_publish_result_t result = { 0, 0, 0, 0 };
    ...
    const uint32_t sent = local != NULL
        ? room_topic_publish(local->tree, topic, topic_len, data, len, binary, except_id,
                           &local_body)
        : 0;
    ...
    (void) room_atomic_u64_add(&hub->posted,  (int64_t) posted);
    (void) room_atomic_u64_add(&hub->skipped, (int64_t) skipped);
    (void) room_atomic_u64_add(&hub->dropped, (int64_t) dropped);
                                ...
                                const char *topic, const size_t topic_len)
{
    ...
    for (int slot = 0; slot < hub->slots_used; slot++) {
        if (slot == own_slot || hub->inbox[slot] == NULL
            || !room_interest_matches(hub, slot, &interest)) {
            continue;
        }

        room_cmd_t *const cmd = room_cmd_new(ROOM_CMD_COUNT, topic, topic_len);
        cmd->query = query;
                        ...
                        const uint32_t timeout_ms)
{
    ...
    const uint32_t local_matches = room_topic_count(local->tree, topic, topic_len);
    ...
    room_query_t *const query = pecalloc(1, sizeof(*query), 1);
    ...
    room_hub_ask_others(hub, local->slot, query, topic, topic_len);
 ...
 * room_hub.h. Implementation: a full target is parked as a {slot,gen} on this
...
static bool room_hub_retry_arm(room_hub_t *hub, room_local_t *local);
static void room_hub_retry_drain(room_hub_t *hub, room_local_t *local);

static retry_entry_t *retry_entry_new(const char *topic, const size_t topic_len,
        room_payload_t *payload, const uint64_t except_id, const uint64_t deadline_ms,
...
{
    room_payload_release(e->payload);
    pefree(e->pending, 1);
    pefree(e, 1);
}
...
static void room_hub_retry_tick_fn(zend_async_event_t *event,
        zend_async_event_callback_t *callback, void *result, zend_object *exception)
{
    ...
    room_hub_retry_drain(hub, local);
...
static bool room_hub_retry_arm(room_hub_t *hub, room_local_t *local)
{
    ...
    if (local->retry_timer == NULL) {
        ...
        retry_cb_t *const cb = (retry_cb_t *)
            ZEND_ASYNC_EVENT_CALLBACK_EX(room_hub_retry_tick_fn, sizeof(*cb));
        ...
        cb->base.dispose = room_hub_retry_cb_dispose;
        cb->hub          = hub;
...
static void room_hub_retry_timer_dispose(room_local_t *local)
{
    if (local->retry_timer == NULL) {
        return;
    }
...
static void room_hub_retry_drain(room_hub_t *hub, room_local_t *local)
{
    ...
    while (e != NULL) {
        ...
        for (uint32_t r = 0; r < e->npending; r++) {
            ...
            room_cmd_t *const cmd = room_cmd_new(ROOM_CMD_PUBLISH, e->topic, e->topic_len);
            cmd->payload   = e->payload;
            cmd->except_id = e->except_id;

            /* +1 before the post; the consumer's drain unconditionally releases
             * it (room_hub_drain), so a failed post must -1. */
    ...
    if (delivered != 0) (void) room_atomic_u64_add(&hub->retry_delivered, (int64_t) delivered);
    if (gone      != 0) (void) room_atomic_u64_add(&hub->retry_gone,      (int64_t) gone);
    if (expired   != 0) (void) room_atomic_u64_add(&hub->retry_expired,   (int64_t) expired);

    /* The tick's caller pauses the timer when this leaves the queue empty. */
}

/* Detach teardown (called from room_hub_detach, off hub->admin). Stop the timer
...
static void room_hub_retry_teardown(room_hub_t *hub, room_local_t *local, const bool request_over)
{
    room_hub_retry_timer_dispose(local);
    ...
    if (lost != 0) {
        ...
        (void) room_atomic_u64_add(&hub->retry_shutdown, (int64_t) lost);
    }
}
...
static bool room_hub_retry_enqueue(room_hub_t *hub, room_local_t *local, retry_entry_t *e)
{
    ...
    if (UNEXPECTED(!room_hub_retry_arm(hub, local))) {
...
static room_hub_publish_result_t room_hub_send_fanout(room_hub_t *hub, const room_local_t *local,
        const char *topic, const size_t topic_len,
        const char *data, const size_t len, const bool binary, const uint64_t except_id,
        retry_target_t *full, uint32_t *nfull, room_payload_t **payload)
{
    room_hub_publish_result_t result = { 0, 0, 0, 0 };
    ...
    if (local != NULL) {
        result.served = room_topic_publish(local->tree, topic, topic_len, data, len, binary,
    ...
    /* The local walk's body carries on to the other workers (room_hub_publish's
...
room_hub_send_result_t room_hub_try_send(room_hub_t *hub, const char *topic, const size_t topic_len,
        const char *data, const size_t len, const bool binary, const uint64_t except_id,
        const uint32_t timeout_ms, const uint32_t queue_max)
{
    room_hub_send_result_t result = { ROOM_HUB_SEND_OK, 0, 0, 0 };

    if (hub == NULL) {
        result.status = ROOM_HUB_SEND_NO_WORKERS;
        return result;
    }
    ...
    if (nfull == 0) {
        ...
        if (out.workers == 0) {
            ...
            result.status = ROOM_HUB_SEND_NO_TARGETS;
        }
    ...
    if (local == NULL) {
        ...
        result.status  = ROOM_HUB_SEND_NO_QUEUE;
    ...
    if (local->retry_count >= queue_max) {
        ...
        (void) room_atomic_u64_add(&hub->retry_rejected, 1);
        result.status  = ROOM_HUB_SEND_QUEUE_FULL;
        result.pending = nfull;
        return result;
    }
    ...
    if (UNEXPECTED(!room_hub_retry_enqueue(hub, local, e))) {
    ...
    (void) room_atomic_u64_add(&hub->retry_queued, (int64_t) nfull);
...
room_hub_send_result_t room_hub_send(room_hub_t *hub, const char *topic, const size_t topic_len,
        const char *data, const size_t len, const bool binary, const uint64_t except_id,
        const uint32_t timeout_ms, const uint32_t queue_max)
{
    room_hub_send_result_t result = { ROOM_HUB_SEND_OK, 0, 0, 0 };
    ...
    if (coroutine == NULL || ZEND_ASYNC_IS_SCHEDULER_CONTEXT) {
        result.status = ROOM_HUB_SEND_NO_CONTEXT;
        return result;
    }
    ...
    if (cancelled) {
        ...
        result.status  = ROOM_HUB_SEND_CANCELLED;
        result.pending = entry_pending;
    } else if (entry_shutdown) {
        result.status  = ROOM_HUB_SEND_SHUTDOWN;   /* our worker detached — not a timeout */
        result.pending = entry_pending;
    } else if (entry_pending == 0) {
        result.status  = ROOM_HUB_SEND_OK;
        result.pending = 0;
    } else {
        result.status  = ROOM_HUB_SEND_EXPIRED;
        result.pending = entry_pending;
    }
 ...
 * it sets is read under hub->admin by room_hub_post_locked. */
PHP_FUNCTION(room_hub_test_force_post_full)
{
    zend_long n;
...
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_room_hub_test_force_post_full, 0, 1, IS_VOID, 0)
    ZEND_ARG_TYPE_INFO(0, n, IS_LONG, 0)
ZEND_END_ARG_INFO()

static const zend_function_entry room_hub_test_functions[] = {
    ZEND_RAW_FENTRY(ZEND_NS_NAME("TrueAsync", "__test_force_topic_post_full"),
                    zif_room_hub_test_force_post_full,
                    arginfo_room_hub_test_force_post_full, 0, NULL, NULL)
    ZEND_FE_END
};

void room_hub_test_register(const int module_type)
{
    zend_register_functions(NULL, room_hub_test_functions, NULL, module_type);
}
#else
void room_hub_test_register(const int module_type)
{
