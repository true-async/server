...
6. Создать `src/fs_util.c` + `include/fs_util.h`. Перенести `slurp_fd`.
7. Удалить `src/static/http_static_etag.{c,h}`, `src/static/http_static_mime.{c,h}`.
8. Обновить `config.m4`, регенерация билда, прогон тестов.
